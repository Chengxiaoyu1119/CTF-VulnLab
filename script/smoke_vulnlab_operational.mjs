...
try {
  ...
  assert.ok(invitationRecords.body.some(item => item.id === invitation.body.id && item.status === 'active'))
  ...
  const revokedRecords = await request(server.baseUrl, '/api/auth/invitations', { headers: { cookie: session.cookie } })
  assert.ok(revokedRecords.body.some(item => item.id === revocableInvitation.body.id && item.status === 'revoked'))
  audits = (await request(server.baseUrl, '/api/audit', { headers: { cookie: session.cookie } })).body
  ...
  assert.equal(afterInvitationDelete.body.some(item => item.id === revocableInvitation.body.id), false)
  const batchInvitations = await Promise.all([
    request(server.baseUrl, '/api/auth/invitations', { method: 'POST', headers: { cookie: session.cookie, 'x-csrf-token': session.csrfToken } }),
    request(server.baseUrl, '/api/auth/invitations', { method: 'POST', headers: { cookie: session.cookie, 'x-csrf-token': session.csrfToken } }),
  ])
  const batchInvitationDelete = await fetch(server.baseUrl + '/api/auth/invitations', { method: 'DELETE', headers: { cookie: session.cookie, 'x-csrf-token': session.csrfToken, 'content-type': 'application/json' }, body: JSON.stringify({ ids: batchInvitations.map(item => item.body.id) }) })
  assert.equal(batchInvitationDelete.status, 200)
  assert.equal((await batchInvitationDelete.json()).deleted, 2)
  const deletedAuditRecord = await fetch(server.baseUrl + '/api/audit/' + revokeAudit.id, { method: 'DELETE', headers: { cookie: session.cookie, 'x-csrf-token': session.csrfToken } })
  assert.equal(deletedAuditRecord.status, 200)
  const auditBatch = (await request(server.baseUrl, '/api/audit', { headers: { cookie: session.cookie } })).body.filter(item => item.action === 'invitation.create').slice(0, 2)
  const batchAuditDelete = await fetch(server.baseUrl + '/api/audit', { method: 'DELETE', headers: { cookie: session.cookie, 'x-csrf-token': session.csrfToken, 'content-type': 'application/json' }, body: JSON.stringify({ ids: auditBatch.map(item => item.id) }) })
  assert.equal(batchAuditDelete.status, 200)
  assert.equal((await batchAuditDelete.json()).deleted, auditBatch.length)
  const afterAuditDelete = await request(server.baseUrl, '/api/audit', { headers: { cookie: session.cookie } })
  assert.equal(afterAuditDelete.body.some(item => item.id === revokeAudit.id), false)
