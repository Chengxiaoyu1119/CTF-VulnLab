...
assert.notEqual(new URL(instance.endpoint).origin, new URL(baseUrl).origin)
assert.notEqual(new URL(instance.endpoint).port, new URL(baseUrl).port)
...
const runtimePath = new URL(instance.endpoint).pathname.replace(/\/$/, '')
...
await assert.rejects(() => fetch(instance.endpoint), 'native PHP endpoint should stop after instance destruction')
